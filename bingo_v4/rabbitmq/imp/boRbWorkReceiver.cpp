...
/* 
 * File:   boRbWorkReceiver.cpp
 * Author: root
 * 
 * Created on 2017年9月20日, 下午1:32
 */

#include "boRbWorkReceiver.h"
...
boRbWorkReceiver::boRbWorkReceiver( boRbConnectDetail& conn , boLogHandler* logger ) :
     boRbReceiver( conn , logger ) {
}

boRbWorkReceiver::~boRbWorkReceiver( ) {
}

void boRbWorkReceiver::connet_to_server( ) {
        ...
        while ( true ) {
...

                {
                        ...
                        channel.setQos( 1 ) ;
                        channel.declareQueue( conn_.routingkey[0].c_str( ) , AMQP::durable ) ;
                        ...
                        channel.consume( conn_.routingkey[0].c_str( ) )
