...
#include "../../bingo_v4/rabbitmq/imp/boRbWorkReceiver.h"
#include "../../bingo_v4/rabbitmq/imp/boRbWorkSendor.h"
...
// two receiver:  02 00 00 00 ff
//                      04 00 00 00 ff
//                      06 00 00 00 ff

BOOST_AUTO_TEST_SUITE( test_rb_worker )
...
class my_rev : public boRbWorkReceiver {
public:

        my_rev( boRbConnectDetail& conn , boLogHandler* logger ) : boRbWorkReceiver( conn , logger ) {
...
void run_rev( int n ) {
        ...
        conn.pwd = "guest" ;
        conn.host = "/" ;
        conn.routingkey.push_back( string( "test_work" ) ) ;
        conn.key = "woker" + lexical_cast<string>( n ) ;
...
BOOST_AUTO_TEST_CASE( t_snd ) {
        ...
        conn.routingkey.push_back( string( "test_work" ) ) ;
        conn.key = "woker" ;
        boRbWorkSendor snd( conn , boActionFactory::make_console_logger( ) ) ;
